...
import { DeepSeekTaskToolNormalizer } from '../normalization/DeepSeekTaskToolNormalizer';
...
export type {
  DeepSeekSessionTimeCandidate,
  DeepSeekSessionTimeFingerprint,
} from './DeepSeekSessionRecovery';
export {
  recoverSDKSessionIdByTime,
  selectDeepSeekSessionRecoveryCandidate,
} from './DeepSeekSessionRecovery';
export type {
  AsyncSubagentResult,
  ResolvedAsyncStatus,
...
export async function loadSDKSessionMessages(
  ...
  const taskToolNormalizer = new DeepSeekTaskToolNormalizer();
  ...
  if (toolUseResults.size > 0 || asyncSubagentResults.size > 0) {
    ...
    for (const msg of chatMessages) {
      ...
      for (const toolCall of msg.toolCalls) {
        if (!isDeepSeekSubagentToolName(toolCall.name)) continue;
...
function normalizeTaskToolCalls(
  message: ChatMessage,
  normalizer: DeepSeekTaskToolNormalizer,
