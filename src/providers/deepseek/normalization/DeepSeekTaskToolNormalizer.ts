...
type DeepSeekTaskToolName = typeof CLAUDE_TASK_TOOL_NAMES[number];
...
export interface NormalizedDeepSeekTaskToolCall {
...
export interface DeepSeekTaskToolResultOptions {
 ...
 */
export class DeepSeekTaskToolNormalizer {
  ...
  normalizeToolUse(
    ...
    rawName: string,
    rawInput: Record<string, unknown>,
  ): NormalizedDeepSeekTaskToolCall | null {
    if (!isDeepSeekTaskToolName(rawName)) return null;
  ...
  normalizeToolResult(
    toolCallId: string,
    rawOutput: unknown,
    options: DeepSeekTaskToolResultOptions = {},
  ): NormalizedDeepSeekTaskToolCall | null {
  ...
  private getOrCreateCall(toolCallId: string, rawName: DeepSeekTaskToolName): TaskCallState {
  ...
  private buildNormalizedCall(
    call: TaskCallState,
    rawOutput?: unknown,
  ): NormalizedDeepSeekTaskToolCall {
...
function isDeepSeekTaskToolName(name: string): name is DeepSeekTaskToolName {
