import {
  DEEPSEEK_HARNESS_SETTINGS_PATH,
...
export type StoredDeepSeekHarnessSettings = DeepSeekHarnessSettings;
...
function normalizeChatViewPlacement(
  ...
  return DEFAULT_SETTINGS.chatViewPlacement;
}
...
function normalizeEnableDualPane(value: unknown): boolean {
  return typeof value === 'boolean'
    ? value
    : DEFAULT_SETTINGS.enableDualPane;
}

function normalizeEnableFilePane(value: unknown): boolean {
  return typeof value === 'boolean'
    ? value
    : DEFAULT_SETTINGS.enableFilePane;
}
...
export class DeepSeekHarnessSettingsStorage {
  constructor(private adapter: VaultFileAdapter) {}

  async load(): Promise<StoredDeepSeekHarnessSettings> {
    ...
    if (
      settingsPath !== DEEPSEEK_HARNESS_SETTINGS_PATH
      || (
  ...
  async save(settings: StoredDeepSeekHarnessSettings): Promise<void> {
    ...
    await this.adapter.write(DEEPSEEK_HARNESS_SETTINGS_PATH, content);
    await this.deleteLegacyFileIfPresent();
  }

  async exists(): Promise<boolean> {
    if (await this.adapter.exists(DEEPSEEK_HARNESS_SETTINGS_PATH)) {
      return true;
    }
  ...
  async update(updates: Partial<StoredDeepSeekHarnessSettings>): Promise<void> {
    const current = await this.load();
    await this.save({ ...current, ...updates });
  }

  private getDefaults(): StoredDeepSeekHarnessSettings {
    return DEFAULT_SETTINGS;
  }

  private async getLoadPath(): Promise<string | null> {
    if (await this.adapter.exists(DEEPSEEK_HARNESS_SETTINGS_PATH)) {
      return DEEPSEEK_HARNESS_SETTINGS_PATH;
    }
